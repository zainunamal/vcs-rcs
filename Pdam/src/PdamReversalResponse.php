...
/**
 * Description of PdamReversalResponse
 *
 * @author yudoh
 */
class PdamReversalResponse extends CISO8583Parser
{
    ...
    private function SplitPrivateData($stream)
    {
        ...
        $splitter->SetBlockRepeatAssocNameArray(array('repbillid', 'repbilrefnum', 'billperiod', 'duedate', 'billamount', 'stamp', 'vat', 'penalty', 'danameter', 'admincharge', 'garbageretribution', 'installment'));
    ...
    private function SplitPrivateData2($stream)
    {
        ...
        $splitter->SetBlockSingleLengthArray(array(30, 1));
        $splitter->SetBlockSingleAssocNameArray(array('address', 'bs'));

        $splitter->SetBlockRepeatLengthArray(array(12, 12, 12));
        $splitter->SetBlockRepeatAssocNameArray(array('danameter', 'pdam_adm', 'waste'));
        $splitter->SetRepeatKey('bs');
        $splitter->Extract();

        $this->privateDataSingle = $splitter->GetComponentSingleArray();
        $this->privateDataRepeat = $splitter->GetComponentRepeatArray();
        //var_dump($this->privateDataRepeat);
