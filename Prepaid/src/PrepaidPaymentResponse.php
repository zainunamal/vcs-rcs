...
/**
 * Description of PrepaidPaymentResponse
 *
 * @author RCS
 */
class PrepaidPaymentResponse extends CISO8583Parser
{
    ...
    private function SplitPrivateData($stream)
    {
        ...
        $splitter->SetBlockHeaderLengthArray(array(7, 11, 12, 1, 32, 32, 8, 25, 4,
            9, 1, 1, 10, 1, 10, 1, 10, 1, 10, 1, 10, 1, 12, 1, 10, 20));
        $splitter->SetBlockHeaderAssocNameArray(array('switcherid', 'msn', 'sid',
            'flag', 'plnrefnum', 'swrefnum', 'vendingrefnum', 'name', 'segment',
            'pcc', 'opt', 'mu_ac', 'ac', 'mu_sd', 'sd', 'mu_vtx', 'vtx', 'mu_ptx',
            'ptx', 'mu_pi', 'pi', 'mu_pp', 'pp', 'mu_kwh', 'kwh', 'token'));
        $splitter->Extract();
