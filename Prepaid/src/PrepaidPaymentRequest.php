...
/**
 * Description of PrepaidPaymentRequest
 *
 * @author RCS
 */
class PrepaidPaymentRequest extends CISO8583Message
{
    public function __construct()
    {
        ...
        //set defaults
        $this->SetValueForDataElement(0, 2200); //mti for payment
    ...
    private function ConstructSecondPrivateData($aPriv)
    {
        $sPriv = '';
        $sPriv .= $aPriv['dc']; //Distribution Code
        $sPriv .= $aPriv['su']; //Service Unit
        $sPriv .= str_pad($aPriv['sup'], 15, " ", STR_PAD_RIGHT); //Service Unit Phone
        $sPriv .= str_pad($aPriv['max'], 5, "0", STR_PAD_LEFT); //Max KWH Limit
        $sPriv .= $aPriv['trepeat']; //Total Repeat

        for ($i = 0; $i < trim($aPriv['trepeat']); $i++) {
            $idx   = $i + 1;
            $sPriv .= str_pad($aPriv['ppunsold'][$idx], 11, "0", STR_PAD_LEFT); //Power Purchase Unsold
        }
        return $sPriv;
    }
